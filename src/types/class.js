...
class DocumentedClass extends DocumentedItem {
	...
	serializer() {
		return {
			...
			props: this.props.size > 0 ? Array.from(this.props.values()).map((prop) => prop.serialize()) : undefined,
			methods: this.methods.size > 0 ? Array.from(this.methods.values()).map((method) => method.serialize()) : undefined,
			events: this.events.size > 0 ? Array.from(this.events.values()).map((event) => event.serialize()) : undefined,
